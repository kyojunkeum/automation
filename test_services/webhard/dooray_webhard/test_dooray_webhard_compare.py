...
def test_dooray_pattern_drive(request):
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto("https://ewalkerdlp.dooray.com/drive")
            time.sleep(3)

            # # 파일 첨부
            # page.get_by_role("button", name="파일 업로드").click()
            # page.get_by_role("button", name="파일 업로드").set_input_files("pattern.docx")
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.step("Dooray drvie Keyword Test")
@pytest.mark.dependency(name="dooray_keyword_drive")
def test_dooray_keyword_drive(request):
    with sync_playwright() as p:
        # 저장된 세션 상태를 로드하여 브라우저 컨텍스트 생성
        session_path = os.path.join("session", "dooraystorageState.json")
        browser = p.chromium.launch(headless=True)
        ...
        try:
            ...
            with page.expect_file_chooser() as fc_info:
                page.get_by_role("button", name="파일 업로드").click()
            file_chooser = fc_info.value
            file_chooser.set_files(r"D:/dlp_new_automation/test_files/keyword_test.docx")
...
@pytest.mark.dependency(
  depends=[
    "dooray_login",
    "dooray_pattern_drive",
    "dooray_keyword_drive"
  ]
)

@allure.severity(allure.severity_level.CRITICAL)
@allure.step("Dooray drive Dlp Logging check")
def test_compare_result_dooray_drive():
    with sync_playwright() as p:
        # 브라우저 실행
        browser = p.chromium.launch(headless=True)
        ...
        try:
            ...
            page.get_by_text("[웹하드] 두레이 드라이브").click()
            # 검색 클릭
            ...
            test_cases = [
                {"row_index": 3, "expected": {"pattern_count": "14", "keyword_count": "0", "file_count": "1"}},  # 키워드 로깅
                {"row_index": 1, "expected": {"pattern_count": "0", "keyword_count": "1", "file_count": "1"}},  # 첨부파일 로깅
            ]
            ...
            # 실패 시 스크린샷 저장
            # 실패 시 스크린샷 경로 설정
            screenshot_path = get_screenshot_path("test_dooray_drive")  # 공통 함수 호출
            page.screenshot(path=screenshot_path, type="jpeg", quality=80)
            # page.screenshot(path=screenshot_path, full_page=True)
            print(f"Screenshot taken at : {screenshot_path}")
            allure.attach.file(screenshot_path, name="dooray_drive_failure_screenshot",
